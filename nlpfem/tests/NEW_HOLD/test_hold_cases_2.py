...
TOL = 10e-5
time = 0
for n in range(0, 5): # LOOP OVER TIME
    ...
    if not any(gap_violation):
        ...
        while np.linalg.norm(extended_residual) > TOL:
            ...
            # NOTE: Only the residula associated with the constraint nodes is not going to zero
            ...
            delta_u = csd.DM(delta_sol[0:mesh.num_dofs,0]).full()
            delta_lag_mult = csd.DM(delta_sol[mesh.num_dofs::,0]).full()
