...
from nlpfem.visual import plot_mesh, make_gif, plot_stress
...
# ======= AUTODIFF RESIDUAL
R, S_avg_map, E_avg_map = mesh.compute_stress_divergence_vector(uhat=uhatnp1_sym)

# get the element stress tensors
...
plot_stress(mesh=mesh,
            ...
            time=0)
fs_n = f'gifs/02_stress/stress/it_0.png'
f_stress_names.append(fs_n)
...
for n in range(0, N): # LOOP OVER TIME
    ...
    plot_mesh(mesh=mesh, 
              ...
              show_wire=True)
    plt.tight_layout()
    fn = f'gifs/02_stress/it_{n+1}.png'
    fnames.append(fn)
    plt.savefig(fn, dpi=300)
    plt.close()

    # ===== plot the stress at time tnp1 ====
    ...
    plot_stress(mesh=mesh,
                ...
                S_avg_vecs=S_avg_vecs,
                time=time)
    fs_n = f'gifs/02_stress/stress/it_{n+1}.png'
    f_stress_names.append(fs_n)
    plt.savefig(fs_n,dpi=300)
...
plt.grid()
plt.legend()
plt.savefig(f'gifs/02_stress/lm_gap_time.png', dpi=300)


# make a gif
make_gif(gif_path='gifs/02_stress/hold_test.gif',
         filenames=fnames,
         duration=0.0002)

# make a gif
make_gif(gif_path='gifs/02_stress/stress/stress_plots.gif',
         filenames=f_stress_names,
         duration=0.0002)
